...
from evaluate import _evaluate_casia_b, evaluate, _evaluate_OUMVLP, _evaluate_YOLO
...
def main(opt):
    ...
    elif opt.dataset == "YOLO":
        dataset = YOLOPose(
            opt.train_data_path,
            sequence_length=opt.sequence_length,
            transform=transform,
        )
        dataset_valid = YOLOPose(
            opt.valid_data_path,
            sequence_length=opt.sequence_length,
            transform=ThreeCenterSequenceTransform(
                transform=val_transform, sequence_length=opt.sequence_length),
    ...
    # Trainer
    print("Số lượng batch trong train_loader:", len(train_loader))
...
if __name__ == "__main__":
    ...
    if opt.dataset == "OUMVLP":
        ...
        opt.evaluation_fn = _evaluate_YOLO
    else:
