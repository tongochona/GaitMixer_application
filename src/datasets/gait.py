...
class YOLOPose(Dataset):
    def __init__(self, csv_path, sequence_length=30, transform=None):
        ...
        # Tìm các chuỗi frame đủ dài
        self.valid_sequences = []
        for (person, video), frames in self.data_dict.items():
            sorted_frames = sorted(frames.keys())
            if len(sorted_frames) >= sequence_length:
                for i in range(len(sorted_frames) - sequence_length + 1):
                    seq_frames = sorted_frames[i : i + sequence_length]
                    self.valid_sequences.append((person, video, seq_frames))

        self.targets = self.valid_sequences
        print(f"✅ Số lượng valid sequences: {len(self.valid_sequences)}")

    def __len__(self):
        return len(self.valid_sequences)

    def __getitem__(self, index):
        person, video, seq_frames = self.valid_sequences[index]

        # Trích xuất chuỗi keypoints liên tiếp
        data = np.array([self.data_dict[(person, video)][frame] for frame in seq_frames], dtype=np.float32)

        if self.transform is not None:
            data = self.transform(data)

        target = (person, video, seq_frames[0], seq_frames[-1])
        return data, target
